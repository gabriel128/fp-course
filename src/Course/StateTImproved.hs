...
module Course.StateTImproved where
...
liftTwice :: Functor f => (a -> b) -> f (f a) -> f (f b)
liftTwice = (<$>) . (<$>)
...
instance Functor f => Functor (StateT s f) where
  ...
  f <$> (StateT fas) = StateT $ (fmap . mapFst) f . fas
...
instance Monad f => Applicative (StateT s f) where
  ...
  (StateT fab) <*> (StateT sfa) = StateT stateF
      where
        stateF s = do
          (f, s') <- fab s
          (a, s'') <- sfa s'
...
instance Monad f => Monad (StateT s f) where
  ...
  f =<< (StateT sfa) = StateT stateF
     where
       stateF s = sfa s >>= uncurry runStateT . mapFst f
...
execT state = fmap snd . runStateT state

-- | Run the `State` seeded with `s` and retrieve the resulting state.
exec' :: State' s a -> s -> s
exec' state = runExactlyOne . execT state

-- | Run the `StateT` seeded with `s` and retrieve the resulting value.
evalT :: Functor f => StateT s f a -> s -> f a
evalT state = fmap fst . runStateT state
