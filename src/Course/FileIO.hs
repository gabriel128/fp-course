...
printFile path chars = putStrLn ("The path: " ++  path ++ ". The chars: " ++ chars)

-- Given a list of (file name and file contents), print each.
-- Use @printFile@.
printFiles :: List (FilePath, Chars) -> IO ()
printFiles = voidSequenceMap $ uncurry printFile
...
run :: Optional FilePath -> IO ()
run Empty = putStrLn "Something went wrong"
run (Full path) = readFile path >>=
  \file -> getFiles (lines file) >>=
