...
filter f = foldRight reducer Nil
  where
    reducer x acc
      | f x = x :. acc
      | otherwise = acc
...
seqOptional = foldRight reducer (Full Nil)
  where
    reducer (Full x) (Full acc) = Full (x :. acc)
    reducer Empty _ = Empty
    reducer _ Empty = Empty
-- seqOptional = foldRight reducer (Full Nil)
--   where
--     reducer x acc =  acc P.>>=
--                      \y -> x P.>>=
...
find p = maybeFirst . filter p
  where
    maybeFirst Nil = Empty
    maybeFirst (x :. _) = Full x
